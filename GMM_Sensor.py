#
# Gaussian Mixture Models on Sensor Readings from a wall-following robot with 4 readings dataset
...
class GMM:
    ...
    def run(self):
        ...
        for iter in range(self.iterations):
            ...
            var_c = []
            for c in range(len(r[0])):
                var_c.append((1/m_c[c])*np.dot(((np.array(r[:,c]).reshape(5456,1))*(self.X.reshape(len(self.X),1)-self.mu[c])).T,(self.X.reshape(len(self.X),1)-self.mu[c])))
...
def main():
	data = pd.read_csv('data/sensor_readings_4.csv')
	...
	for c in range(0,len(data.columns) - 1):
		...
		x = np.linspace(x.min(), x.max(), 5456, endpoint=False)
